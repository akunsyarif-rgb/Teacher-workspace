...
import { Home, BookOpen, Calendar, Users, Wallet } from "lucide-react";
...
const KAS_KELAS_ITEM = { href: "/kas-kelas", label: "Kas Kelas", icon: Wallet };
...
export default function BottomNav() {
  ...
  const navItems = teacherProfile?.homeroomClassName
    ? [...BASE_NAV_ITEMS, KAS_KELAS_ITEM]
    : BASE_NAV_ITEMS;
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-100 shadow-[0_-2px_10px_rgba(0,0,0,0.04)]">
      <div className={`max-w-5xl mx-auto grid ${navItems.length === 5 ? 'grid-cols-5' : 'grid-cols-4'}`}>
        {navItems.map(({ href, label, icon: Icon }) => {
          ...
          return (
            <Link
              key={href}
              href={href}
              className={`flex flex-col items-center justify-center gap-1 py-2.5 text-[10px] font-bold transition-colors relative ${
